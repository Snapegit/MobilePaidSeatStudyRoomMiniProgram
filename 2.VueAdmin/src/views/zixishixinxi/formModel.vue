				...
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="预约状态" prop="yuyuezhuangtai">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.yuyuezhuangtai?true:false"
								v-model="form.yuyuezhuangtai" 
								placeholder="请选择预约状态"
								>
								<el-option v-for="(item,index) in yuyuezhuangtaiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.xiangqing = res.data.data.xiangqing?(res.data.data.xiangqing.replace(reg,'../../../cl95234104/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='yuyuezhuangtai'){
					form.value.yuyuezhuangtai = row[x];
					disabledForm.value.yuyuezhuangtai = true;
					continue;
				}
			}
			if(row){
		...
		yuyuezhuangtaiLists.value = "已预约,未预约".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
