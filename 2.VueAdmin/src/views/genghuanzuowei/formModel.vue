		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="新座位号" prop="xinzuoweihao">
							<el-input class="list_inp" v-model="form.xinzuoweihao" placeholder="新座位号"
								 type="text" 								:readonly="!isAdd||disabledForm.xinzuoweihao?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="24">
						<el-form-item label="更换时间" prop="genghuanshijian">
							<el-date-picker
								class="list_date"
								v-model="form.genghuanshijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.genghuanshijian?true:false"
								placeholder="请选择更换时间" />
						</el-form-item>
					</el-col>
				</el-row>
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.genghuanshijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='xinzuoweihao'){
					form.value.xinzuoweihao = row[x];
					disabledForm.value.xinzuoweihao = true;
					continue;
				}
				if(x=='zixishileixing'){
				...
				if(x=='genghuanshijian'){
					form.value.genghuanshijian = row[x];
					disabledForm.value.genghuanshijian = true;
					continue;
				}
			}
			if(row){
